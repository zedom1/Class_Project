...
BubbleSort * BubbleSort::bubblesort = nullptr;
 ...
 */
BubbleSort::BubbleSort(int num, int*array)
{
    ...
    connect(event->swapTimer,&QTimer::timeout, this,&BubbleSort::swap);
    connect(event->wholeTimer, &QTimer::timeout, this, &BubbleSort::handleRecord);
 ...
 */
void BubbleSort::handleArray(int *array)
{
    int minx = 100;
    int maxn = 0;
    int * temarray;
...
{
    if(count==0){
        if(itemList[index1]->rect->getColorMode()!=Rect::MAXN)
            itemList[index1]->rect->setColorMode(Rect::SWAP);
        if(itemList[index2]->rect->getColorMode()!=Rect::MAXN)
...
void BubbleSort::getRecord()
{
    ...
    for(int i=0;i<numItem-1;i++){
        for(int j=numItem-1;j>i;j--){
            ...
            recordList->addRecord(0,1,j-1);
            if(temarray[j]<temarray[j-1]){
                recordList->addRecord(0,2,j);
                recordList->addRecord(1,j,j-1);
                int temp=temarray[j];
                temarray[j]=temarray[j-1];
                temarray[j-1]=temp;
            }
            else{
        ...
        recordList->addRecord(0,3,i);
    }
    recordList->addRecord(0,2,numItem-1);
    delete []temarray;
}
