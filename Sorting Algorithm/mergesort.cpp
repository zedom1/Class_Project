 ...
 */
MergeSort::MergeSort(int num, int*array)
{
...
void MergeSort::restart()
{
    ...
    while(-1!=current->type){
        ...
        if(current->type==5)
        {
            int l=current->attribute1;
            int r=current->attribute2;
            for(int i=l;i<r+1;i++)
            {
                itemList[i]=current->b[i-l];
            }
        }
